...
<div id="issue_actions_container" class="issuePanelContainer">
    ...
    <div id="comment-<%=comm.getId()%>-open" style="display: block;">
        <div class="actionContainer">
            <div class="action-links subText smallgrey">
                [
                <a class="smallgrey" href="<%=request.getContextPath()%>/jsp/frontend/comment/updateCommentDetail.jsp?commentId=<%=comm.getId()%>&issueRCKey=<%=issueRC.getIssueKey()%>" id="edit_comment_<%=comm.getId()%>">编辑</a>
                |
                <a class="smallgrey" href="<%=request.getContextPath()%>/jsp/frontend/comment/deleteComment.jsp?commentId=<%=comm.getId()%>&issueRCKey=<%=issueRC.getIssueKey()%>" id="delete_comment_<%=comm.getId()%>">删除</a>
                |
                <a class="smallgrey" onclick="toggleDivsWithCookie('comment-<%=comm.getId()%>-closed', 'comment-<%=comm.getId()%>-open'); return false;" href="#">« Hide</a>
                ]
            </div>
            <div class="action-details">
                ...
                -
                <span class="subText">
                    <span class="date"><%=comm.getCreateDate()%></span>
                </span>
            </div>
    ...
    <div id="comment-<%=comm.getId()%>-closed" style="display: none;">
        <div class="actionContainer">
            <div class="action-links subText smallgrey">
                [
                <a class="smallgrey" onclick="toggleDivsWithCookie('comment-<%=comm.getId()%>-open', 'comment-<%=comm.getId()%>-closed'); return false;" href="#">show »</a>
                ]
            </div>
            <div class="action-details">
                <a href="#"><%=comm.getAuthor()%></a>
                -
                <span class="subText">
                    <span class="date"><%=comm.getCreateDate()%>
                        <font color="#000000">
                            <b><%=comm.getCommentContent()%></b>
                        </font>
                    </span>
                </span>
            </div>
