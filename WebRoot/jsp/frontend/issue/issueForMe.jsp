...
<table class="tableBorder maxWidth" width="100%" cellspacing="1"
	...
	<tr class="rowHeader">
		<td class="colHeaderLink" colspan="4">
			开发的问题:
			<b><a
				href='<%=path%>/jsp/frontend/issue/issueNavigator.jsp?action=forMe'>分配给我的</a>
				<%
					if (issueList != null) {
					if (issueList.size() < 10) {
						out.println("<small>(显示" + issueList.size() + "中的"
								+ issueList.size() + ")");
					} else {
	...
	<tr class="rowNormal DFD-3-tr">
		<td nowrap="" width="1%">
			<a
				href=" <%=path%>/jsp/frontend/issue/issueDetailLayout.jsp?action=forMe&issueKey=<%=issue.getIssueKey()%>">
				<img height="16" width="16" border="0" align="absmiddle"
					title="<%=issue.getIssueTypeName()%>"
					alt="<%=issue.getIssueTypeName()%>"
					src=" <%=request.getContextPath()
							+ issue.getIssueTypeIcon()%>" />
			</a>
		</td>
		...
		<td width="100%">
			<a style="text-decoration: none;" 
				href=" <%=path%>/jsp/frontend/issue/issueDetailLayout.jsp?action=forMe&issueKey=<%=issue.getIssueKey()%>"><%=issue.getName()%></a>
		</td>
		<td nowrap="" width="1%">
			<img height="16" width="16" border="0" align="absmiddle"
				title="<%= issue.getPriorityLevel() %>" alt="Major"
				src=" <%=request.getContextPath()
							+ issue.getPriorityLevelIcon()%> " />
		</td>
