	...
	<tbody>
		...
		<%
			if (groupList == null || groupList.size() == 0) {
				System.out.println("=========groupList is null===");
			} else {
				for (int i = 0; i < groupList.size(); i++) {
					Group groupView = (Group) groupList.get(i);
		%>
		<tr bgcolor="#ffffff">
			<td valign="top"><a
				href="viewGroupDetail.jsp?groupName=<%=groupView.getName()%>"><%=groupView.getName()%></a>
			</td>
			<td valign="top"><%=groupView.getUserCount()%> <font size="1">
			...
			<td nowrap="" valign="top"><a id="del_dsffd"
				href="<%=request.getContextPath()%>/jsp/backend/group/DeleteGroup!default.jsp?groupName=<%=groupView.getName()%>">删除</a>
				| <a id="edit_members_of_dsffd"
				href="BulkEditUserGroups!default.jsp?groupName=<%=groupView.getName()%>">Edit
					Members</a></td>
		</tr>
		<%
			}
