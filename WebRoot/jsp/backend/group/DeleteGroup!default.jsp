...
<body>
	...
	<form
		...
		name="simpleform" method="post" action="DeleteGroup.jsp">
			...
			<tbody>
				<tr bgcolor="#f0f0f0">
					<td class="simpleformheader" colspan="2">
						...
						<p>If you delete a group, all users in that group will be
							removed from the group first.</p>
						<p>Deleting a group will not delete any users.</p>

					</td>
				</tr>
				<tr class="hidden">
					<td><input id="name" type="hidden" value="<%=groupName%>"
						name="groupName" /></td>
				</tr>
				<tr>
					<td class="fullyCentered simpleformfooter" colspan="2">
						...
						onclick="location.href='<%=request.getContextPath()%>/jsp/backend/group/GroupBrowser.jsp'"
						value="取消" name="GroupBrowser.jspa" title="取消 (Alt + `)"
						accesskey="`" /> <%
 	} else {
