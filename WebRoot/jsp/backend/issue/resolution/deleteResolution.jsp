				...
				<form
					...
					<table class="simpleform maxWidth">
						<tbody>
							...
							<tr>
								<td class="simpleformheader" colspan="2">
									<p>Confirm that you want to delete this resolution, and
										specify what is to be done with the issues currently attached
										to it.</p>
									<p>
										There are currently <b><%=count%></b> matching issues, that
										must be changed to another resolution.
									</p>
								</td>
							</tr>
							<tr>
								<td class="fieldLabelArea">New resolution for matching
									issues:</td>
								<td class="fieldValueArea" bgcolor="#ffffff"><select
									id="newId_select" name="newId">
										<%
										...
										%>
								</select> <span id="newId_summary" class="selectDescription" /></td>
							</tr>
							<tr class="hidden">
								<td><input id="id" type="hidden"
									value="<%=resolution.getId()%>" name="id" /></td>
							</tr>
							<tr class="hidden">
								<td><input id="confirm" type="hidden" value="true"
									name="confirm" /></td>
							</tr>
							<tr>
								<td class="fullyCentered simpleformfooter" colspan="2"><input
									id="删除" class="spaced" type="submit" title="按 Alt+S 提交"
									accesskey="S" value="删除" name="删除" /> <input id="cancelButton"
									type="button" onclick="window.history.back();" value="取消"
