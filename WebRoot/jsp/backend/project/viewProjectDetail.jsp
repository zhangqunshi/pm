...
<body>
	<div style="border: 1px solid rgb(187, 187, 187); padding: 2px;">
		<table border="0">
			...
			<tr>
				<td><b>Key: </b><%=project.getProjectKey()%> <br> <b>网址:
				</b> <%
 ...
 %><br> <b>负责人: </b><%=project.getLeader()%> <br> <b>Project
						Roles : </b> <a
					href="<%=request.getContextPath()%>/jsp/backend/project/user/viewProjectUser.jsp?projectId=<%=pid%>">
						View members </a> <br> <b>Permission Scheme: </b> <%=ps.getName()%>
					( <a
					href="<%=path%>/jsp/backend/permission/selectProjectPermissionScheme.jsp?projectId=<%=pid%>">选择</a>
					| <a
					href="<%=path%>/jsp/backend/permission/editPermissions.jsp?schemeId=<%=ps.getId()%>">编辑</a>
					) <br /> <b>描述: </b><%=project.getDescription()%> <br> <%
 	//判断项目类型是否为空
 	if (project.getCategoryId() == null) {
 		...
 		ProjectCategory category = pc.getProjectCategory(project.getCategoryId().getId()); //getId()修改
 ...
 %></td>
	...
	<table>
		<tr>
			<td><a href="<%=path%>/jsp/backend/project/viewProject.jsp">浏览项目</a>
				| <a
	...
	<table>
		<tr>
			<td><%@ include
					file="/jsp/backend/project/component/viewComponents.jsp"%>
			</td>
		</tr>
	</table>
