...
<%
	...
	Project project = pb.getProject(proId);
...
<table>
	...
	<input type="hidden" value="<%=proId%>" name="projectId"
		id="projectId" />
	<input type="hidden" value="true" name="confirm" />
	<tr>
		<td colspan="2"><input type="submit" class="spaced"
			title="按 Alt+S 提交" accesskey="S"
			onclick="location.href='<c:url value="/jsp/backend/project/doDeleteProject.jsp"/>?projectId=<%=proId%>'"
			value="删除" id="删除" name="删除" /> <input type="button"
			onclick="window.history.back();" value="取消" title="取消 (Alt + `)"
			accesskey="`" id="cancelButton" /></td>
	</tr>
</table>