			...
			<tr>
				<td>
					<form
						...
						name="simpleform" method="post" action="../doUpdateProject2.jsp">
							...
							<tbody>
								...
								<tr>
									<td class="simpleformheader" colspan="2">This page allows
										you to select which project category you want this project to
										be in.</td>
								</tr>
								<tr>
									<td class="fieldLabelArea">项目分类:</td>
									<td class="fieldValueArea" bgcolor="#ffffff"><select
										id="pcid_select" name="categoryId">
											<option value="-1">
													None
												</option>
											<%
												for (int i = 0; i < list.size(); i++) {
													ProjectCategory category = (ProjectCategory) list.get(i);
													out.println("<option value=" + category.getId() + ">" + category.getName() + "</option>");

												}
											%>
									</select> <span id="pcid_summary" class="selectDescription" /></td>
								</tr>
								<input id="projecId" type="hidden" value="<%=id%>"
									name="projectId" />
								<tr>
									<td class="fullyCentered simpleformfooter" colspan="2"><input
										id="选择" class="spaced" type="submit" title="按 Alt+S 提交"
										accesskey="S" value="选择" name="选择" /> <input
										id="cancelButton" type="button"
										onclick="window.history.back();" value="取消"
										name="ViewProject.jspa?pid=10022" title="取消 (Alt + `)"
										accesskey="`" /></td>
								</tr>
							</tbody>
						</table>
