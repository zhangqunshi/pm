...
public class PermissionAssigneeBO {
	...
	public List<PermissionAssignee> getAllPermissionAssignee(int schemeId) {
		...
		try {
			tx = session.beginTransaction();
			l = session.createQuery("from PermissionAssignee as p where p.schemeId.id=:id").setInteger("id", schemeId)
					.list();
			tx.commit();
		} catch (Exception e) {
			if (tx != null)
	...
	public boolean exist(PermissionAssignee pa) {
		...
		try {
			tx = session.beginTransaction();
			PermissionAssignee p = (PermissionAssignee) session.createQuery(
					"from PermissionAssignee as p where p.permissionId.id=:pid and p.assigneeType=:at and p.assignee=:a and p.schemeId.id=:sid")
					.setInteger("pid", pa.getPermissionId().getId()).setString("at", pa.getAssigneeType())
					.setString("a", pa.getAssignee()).setInteger("sid", pa.getSchemeId().getId()).uniqueResult();
	...
	public List<PermissionAssignee> getGroupInPermissionAssigneeList(String assigneeType, String assignee) {
		...
		try {
			tx = session.beginTransaction();
			l = session.createQuery("from PermissionAssignee as p where p.assigneeType=:at and p.assignee=:a")
